...
let count = fun k ->
  ...
  in let poket = Bat.take 2 deck
     ...
     and opp_board = Bat.drop 2 deck |> Bat.take 7  
     and opp = Bat.drop 7 deck |> Bat.take 2  
     in 
       if List.exists (is_eq k) poket 
       then 
         if List.for_all (is_lt k) opp_board  
         then     (incr c0 ;                     incr c1) 
         else 
           if (List.exists (is_eq k) board) && (not (List.exists (is_eq k) opp))
           then 
             if (List.for_all (is_eq k) poket) && (not (List.exists (is_eq k) opp)) 
