...
module Evaluations =
  struct
    ...
    let pricePair (xs : hand) =
      let k0 = takeRanks xs |> fun ys -> List.fold_right (fun x acc -> if x == acc then acc else x)
                                                         (List.tl ys)
                                                         (List.hd ys)
      in let k1 = List.filter (fun (r,_) -> r != k0) xs |> priceHigh
         in let k2 = List.filter (fun (r,_) -> r != k0 && r != k1) xs |> priceHigh
            in let k3 = List.filter (fun (r,_) -> r != k0 && r != k1 && r != k2) xs |> priceHigh
               in 3000 * (k0 + 1) + 200 * (k1 + 1) + 13 * (k2 + 1) + k3
    ;;

    let priceDupal (xs : hand) =
      let (n1, n2) = takeRanks xs |> fun ys ->
                                     let ys1 = ys @ [0]
                                     and ys2 = 0 :: ys
    ...
    let priceSet (xs : hand) =
      let n = takeRanks xs |> fun ys ->
                              List.map2 (fun s1 s2 -> if s1 = s2 then s1 else 0)
                                        (ys @ [0])
                                        (0 :: ys) |>
