...
let myfunc = fun _ -> 
  ...
  in let (p1,_) :: (p2,_) :: _  = List.take 2 cs |> List.sort mysort
     and (q1,_) :: (q2,_) :: _  = List.drop 2 cs |> List.take 2 |> List.sort mysort
     in if (r1 > p1 && r1 > q1 && r1 = r2) then true
        else (p1 != p2 && q1 != q2 && r1 > p1 && r1 > q1 && r2 >= p2 && r2 >= q2)
             || ((r1 = p1 && r2 > p2 && p1 > q1) || (q1 > p1 && r1 = q1 && r2 > q2))
             || (r2 > 8 && p1 < 10 && p2 < r2 && q1 < p1 && q2 < r2)
;;

(1 -- 10000) |> Enum.map myfunc |> Enum.map (fun x -> if x then 1 else 0) |> Enum.reduce (+) |>
